...
import functools
from dillingUtils import Layertable, LayertableParser  # pyright: ignore[reportMissingImports]

from createXML import XMLBuilder, XMLElement
...
def createDisplayPacket(name, display, stippleName, lineStyleName, fillColor, outlineColor, fillStyle=None):
    displaypackets[name][display] = DisplayPacket(display = display,
                                                  ...
                                                  fillStyle = fillStyle if fillStyle else "")
            ...
            for groupname, lpp_list in lpp_groups.items():
                if len(lpp_list) == 1:
                    ...
                    with lyp("properties"):
                        ...
                        lyp.short("frame-brightness", str(0))
                        lyp.short("fill-brightness", str(0))
